...
int main() {
    ...
    while (true) {
        ...
        if (msg.type() == Message::MessageType::Check) {
            if (msg.x() == 0 && msg.y() == 0) {
                Message new_msg = Message::create<Message::MessageType::Check>(
                    msg.id(), 1, 0);
                ...
                assert(false);
                   ...
                   Message::MessageType::Connect) {  // give client permission
                                                     // to create user
            int new_id = -1;
            ...
            // client to server
            std::cerr << "Received bad message type (to server from client) = |"
                      << msg.type() << "|\n";
            assert(false);
        }
    }
